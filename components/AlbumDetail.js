...
const AlbumDetail = ({ navigation, title, albumId, style }) => {
  //setea los objetos de estilos definidos
  const { headerContentStyle, headerTextStyle, buttonStyle } = styles;
  //setea el estado de la imagen
  const [photoUrl, setPhoto] = useState(null);

  //Obtiene la primer foto para mostrarla como portada del album
  ...
  //useEffect se utiliza solo una vez para llamar a la función getThumbnail
  useEffect(() => {
    getThumbnail();
  }, []);

  //Utilizamos FontAwewsome para mostrar el icono de flecha
  return (
    <Card style={style}>
